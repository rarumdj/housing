...
export function isPlaceholderStorageUrl(url: string | undefined | null): boolean {
  return typeof url === 'string' && url.startsWith('local://');
}

export function canDisplayMediaInBrowser(url: string | undefined | null): boolean {
...
export function getDisplayPhotos(media: PropertyMedia[] | undefined): PropertyMedia[] {
  if (!media?.length) return [];
  return media
    .filter((m) => {
      if (isVideoMedia(m)) return false;
      if (!canDisplayMediaInBrowser(m.url)) return false;
      return m.type === 'PHOTO' || m.type === 'TOUR_360' || m.type === 'MODEL_3D';
    })
    .sort(mediaOrder);
}

export function getDisplayVideos(media: PropertyMedia[] | undefined): PropertyMedia[] {
  if (!media?.length) return [];
  return media.filter((m) => isVideoMedia(m) && canDisplayMediaInBrowser(m.url)).sort(mediaOrder);
}

/** Panorama / 360 assets for Photo Sphere Viewer. */
export function getPanoramaTourMedia(media: PropertyMedia[] | undefined): PropertyMedia[] {
  if (!media?.length) return [];
...
export function pickListThumbnail(media: PropertyMedia[] | undefined): {
  ...
  if (v?.url) {
    return { url: canDisplayMediaInBrowser(v.url) ? v.url : null, kind: 'video' };
  }
